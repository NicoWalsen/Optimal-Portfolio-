...
#Plot each stocks and the equal weighted portfolio
...
#We will create random list with numbers with 10 rows and 6 columns.
np.random.random(10*6).reshape(10,6) 
#If we run the previous cell once again, we will get different numbers each time. To avoid this issue, we can use ".random.seed()" method.
...
matrix
#We generate the sum of each row for the previous matrix (axis=1). Keepdims = True means that we will mantain the same dimentions of the origin matrix.
...
weights 
weights.sum(axis=1, keepdims=True) #sum of each row is equal to 1.
...
# Plot the stocks and equal weighted portfolio and include 100.000 random portfolios. 
...
#Plot our 100.000 stocks and their Sharpe Ratio, higher sharpe ratios are colored with strong red and lower with strong blue.
#We will also plot 6 initial stocks.
...
port_summary.describe()
weights
msrp = port_summary.Sharpe.idxmax() #We will get the row number with the higher Sharpe Ratio using the method .idmax()
msrp
msrp_p = port_summary.iloc[msrp] #We found the higher Sharpe ratio in the row #76879 with its descriptive statistics.
msrp_p
msrp_w = weights[msrp,:] # With this we obtain the weights for this portfolio.
msrp_w
#Finally we get the weights for each assets of the portfolio with the maximum Sharpe ratio.
